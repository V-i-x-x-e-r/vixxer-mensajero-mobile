...
async function pedir(ruta, opciones = {})
{
  ...
  try
  {
    r = await fetch(`${API_URL}${ruta}`, { ...opciones, signal: control.signal });
  }
  catch (e)
  {
...
export function marcarLeidosGrupo(grupoId, ids)
{
  return conAuth(`/api/grupos/${grupoId}/mensajes/leido`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function subirMediaConProgreso(datos, onProgreso)
{
  const token = await leer(TOKEN);
  return new Promise((resolver, rechazar) =>
  {
    ...
    if (xhr.upload)
    {
      ...
      {
        if (onProgreso && e.total > 0)
        {
          onProgreso(e.loaded / e.total);
        }
      };
    }
    xhr.onload = () =>
    {
      if (xhr.status >= 200 && xhr.status < 300)
      {
        ...
        {
          rechazar(new Error("Respuesta inválida del backend"));
        }
      }
      else
      {
        const err = new Error(`Error ${xhr.status}`);
