...
import { marcarRespaldado, leerConfig, debeRespaldar } from "./respaldoConfig";

export async function obtenerCodigo()
{
...
export async function respaldoAutomatico()
{
  try
  {
    ...
    if (config.destino !== "nube" || !(await debeRespaldar()))
    {
      return;
    }
    if (new Date().getHours() < config.hora)
    {
      return;
    }
    await hacerRespaldo();
