...
export async function cifrarArchivoTrozos(base64Archivo, onProgreso)
{
  ...
  {
    const bytes = decodeBase64(base64Archivo.slice(i * TROZO_B64, (i + 1) * TROZO_B64));
    const sellado = nacl.secretbox(bytes, nonceDeTrozo(nonce, i), clave);
    piezas.push(encodeBase64(enmarcarTrozo(sellado)));
    if (onProgreso)
    {
...
{
  const abierto = nacl.secretbox.open(
    decodeBase64(selladoBase64),
    nonceDeTrozo(decodeBase64(nonceBase64), indice),
    decodeBase64(claveBase64),
  );
  return abierto ? encodeBase64(abierto) : null;
}
