...
const GRUPOS = "vixxer_lista_grupos";

export async function leerCacheGrupos()
{
  try
  {
    const crudo = await AsyncStorage.getItem(GRUPOS);
    return crudo ? JSON.parse(crudo) : null;
  }
  catch (e)
  {
...
export async function guardarCacheGrupos(datos)
{
  try
  {
    await AsyncStorage.setItem(GRUPOS, JSON.stringify(datos));
  }
  catch (e)
  {
  }
